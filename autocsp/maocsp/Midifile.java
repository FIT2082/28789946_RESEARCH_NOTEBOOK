 ...
 * Midifile.java
 *
 * Created on 17 fvrier 2005, 22:24
 */
...
public class Midifile {
    ...
    private int div;                // nombre de divisions lmentaires par noire
    private String nomfich;         // nom du fichier  gnrer
    
    private int[] instrus;          // tableau des instruments utiliss (instrus.get(i) => instrument de la ieme piste )
    private ArrayList[] partoche;    // tableau des diffrentes pistes (chaque piste etant une liste de notes)

    private ArrayList batterie;     // Piste de batterie
    
    private ArrayList metas;        // Liste des mta-vnements prsents (changement de tempo et texte
    
    private int offset;             // Dcalage de la note dans le temps en cours
    ...
    public Midifile(String nom, int trk, int divi) {
        ...
        nomnotes="C 0 C#0 D 0 D#0 E 0 F 0 F#0 G 0 G#0 A 0 A#0 B 0 C 1 C#1 D 1 D#1 E 1 F 1 F#1 G 1 G#1 A 1 A#1 B 1 C 2 C#2 D 2 D#2 E 2 F 2 F#2 G 2 G#2 A 2 A#2 B 2 C 3 C#3 D 3 D#3 E 3 F 3 F#3 G 3 G#3 A 3 A#3 B 3 C 4 C#4 D 4 D#4 E 4 F 4 F#4 G 4 G#4 A 4 A#4 B 4 C 5 C#5 D 5 D#5 E 5 F 5 F#5 G 5 G#5 A 5 A#5 B 5 C 6 C#6 D 6 D#6 E 6 F 6 F#6 G 6 G#6 A 6 A#6 B 6 C 7 C#7 D 7 D#7 E 7 F 7 F#7 G 7 G#7 A 7 A#7 B 7 C 8 C#8 D 8 D#8 E 8 F 8 F#8 G 8 G#8 A 8 A#8 B 8 C 9 C#9 D 9 D#9 E 9 F 9 F#9 G 9 G#9 A 9 A#9 B 9 C 10C#10D 10D#10E 10F 10F#10G 10";
                //|   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |  Ouf c' est bon, pas de dcalages :)
    }

    /** Ajoute une note  la piste trak, en accord avec la suivante ou non*/
    ...
    /** Renvoie le nombre de pistes instrument que contient le fichier  crer*/
    ...
    /** Ajoute un silence  la piste trak de duree divisions*/
    public void AddSilence(int trak, int duree) {
        AddNote(trak,128,duree,false);
    }

    /** Dfinit l' instrument qui jouera la piste trak*/
    ...
    /** renvoie la longueur d' une piste (en nombre de division lmentaires)*/
    ...
    /** Renvoie la longueur de la piste de batterie (en nombre de divisions lmentaires) */
    ...
    /** Modifie le tempo de la musique. Cette mthode ne synchronise pas les pistes. Le changement s' effectue  la fin de la dernire note de la piste la plus longue */
...
//        SyncTracks();
    ...
    /** Ajoute un vnement texte au fichier MIDI*/
