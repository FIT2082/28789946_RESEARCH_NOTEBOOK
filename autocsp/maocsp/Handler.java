...
public class Handler extends DefaultHandler {
    ...
    boolean vardisabled;    // Si on est en train de dcrire un e variable non retenue par l' utilisateur
    int count;              // numro de la balise que l' on traite
    
    DefaultHandler zemod;   // Crateur de  la musique  qui on va repasser tous les vnements SAX...
    ...
    public void startElement(String uri, String nom, String nomnom, Attributes attr) throws SAXException {
        ...
        if (nomnom.equals("new-variable")) {                                    // New-variable => Mise a jour du nombre de variables rencontres
        ...
        if (Partage.notallvar & (nomnom.equals("variable") | nomnom.equals("new-variable") | nomnom.equals("reduce"))) {
            vardisabled=!Partage.selectedvars[Partage.varno(attr.getValue("vident"))];      // On vrifie qu' on n' est pas en train de parler d' une variable non retenue...
        }
        
        
        /* Traitement des diffrents calculs de domaines, profondeur, etc... */
        ...
        if (nomnom.equals("vardomain")) {
            ...
            Partage.domainSize[Partage.currentvar]=Integer.parseInt(attr.getValue("size"));     // WARNING : Cette information peut se rvler inexacte. Penser  prvenir Ludovic  ce sujet.
                                                                                                // Et accessoirement, se fier  la longueur des tableaux gnrs plutot qu' a cette valeur pour viter les bugs...
            ...
            if (Partage.max[Partage.currentvar]-Partage.min[Partage.currentvar] < 256) {        // Si le domaine est compris dans un intervalle de moins de 256 valeurs...
                Partage.isArcCo[Partage.currentvar]=true;                                       // Alors il est lgitime de dire qu' on peut tre en prsence d' une variable sur laquelle l' Arc consistance a t applique
            }
        }
        ...
        if (nomnom.equals("range")) {                                                               
            if (Partage.vardomain) {                                                                // Si cet intervalle dcrit un domaine de variable...
                int from=Integer.parseInt(attr.getValue("from"));
                int to=Integer.parseInt(attr.getValue("to"));
                if (Partage.isArcCo[Partage.currentvar]) {                                          // ... Et qu' on est en prsence d' une variable suceptible d' avoir subi l' arc consistance...
        ...
        /* Et enfin, si les bonnes conditions sont remplies c' est  dire :
         ...
         * alors, on envoie la balise XML  la couche musicale. */
