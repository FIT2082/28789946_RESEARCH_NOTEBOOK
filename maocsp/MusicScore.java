 ...
 * MusicScore.java
 *
 * Created on 27 février 2005, 11:49
 *
 * Cette classe permet de faire toutes les gammes à 7 tons que l' on désire, de définir des accords sur la gamme existante.
 *
 */
...
public class MusicScore {
    ...
    private int tonique;    // Note tonique de la gamme en cours. Mise à jour dès que l' armure ou le mode (mineur ou majeur) est modifié.
    
    private int[] gamme;  // gamme[x] : xieme note de la gamme en cours (modulo l' octave)
    
    private Midifile zemidi;
    
    /** Permet de définir la gamme en cours. Parametres : la tonique et les intervalles entre chaque notes */
    public void SetGamme(int ton, int i1, int i2, int i3, int i4, int i5, int i6) {
        
        int toni=ton;
        while(toni>11) {toni=toni-12;}    // On remet la tonique donnée sur la première octave.
        tonique=toni;
        ...
        zemidi.SyncTracks();            // Pour éviter qu' une piste dans la gamme précédente ne soit jouée avec une autre piste dans la nouvelle gamme, ce serait moche...
    }
    
    /** Renvoie le mode de la gamme actuellement utilisée (true-> mineur, false->majeur)*/
    public boolean GetMinor() {return minor;}
    
    /** Renvoie l' armure de la gamme actuellement utilisée*/
    public int GetArmure() {return armure;}
    
    /** Calcule une gamme ionienne ou dorienne en fonction de l' armure et du mode précédamment défini*/
    ...
    public MusicScore(String nom,int pistes,int div) {
        ...
        gamme=new int[7]; // 7 notes dans une gamme. Je simplifie cette classe au solfege de base, 
                          ...
                          // A ce propos, exit aussi les commas turques, mais même le MIDI n' a pas été prévu pour à la base...
    ...
    /** Renvoie la longueur de la piste zetrak en nombre de divisions élémentaires*/
    public int GetTrkLength(int zetrak) {
        return zemidi.GetTrkLength(zetrak);
    }

    /** Renvoie la longueur de la piste batterie en nombre de divisions élémentaires*/
    public int GetPercLength() {
        return zemidi.GetPercLength();
    }
    
    /** Ajoute un évènement texte au fichier*/
    ...
    /** change d' armure avec la valeur placée en parametre pour les futures notes posées. 
     ATTENTION : Recalcule la gamme automatiquement. Exit toute gamme personalisée avant...*/
    public void ChangeArmure(int zenew) {
        armure=zenew;
        CalculGamme();
    }
    
    /** Change de mode (mineur/majeur) pour les prochaines notes. L' armure reste la même 
     ATTENTION : Recalcule la gamme automatiquement. Exit toute gamme personalisée avant...*/
    ...
    /** permet de changer en même temps l' armure et le mode 
     ATTENTION : Recalcule la gamme automatiquement. Exit toute gamme personalisée avant...*/
    ...
    /** Ajoute une note au morceau, sur la piste indiquée 
     * zenote : degré de la note, trak : piste sur laquelle poser la note, octave : octave de la note,
     * duree : durée de la note en nombre de divisions élémentaires, 
     * accidental : alteration accidentelle de la note (-1 pour bémol, 1 pour diese, 0 pour ne pas altérer la note)
     * */
    ...
    /** Ajoute un accord de nbnotes notes sur la piste trak. POur chaque degré de la gamme, true indique la présence éventuelle de la note dans l' accord*/
    ...
    /** Ajoute un accord sur la piste trak. Cet accord sera uen triade diatonique si nbnotes est égal à 3.
     * Number définit le degré de base de l' accord, renv son renversement*/
    ...
    /** Ajoute un silence de duree divisions élémentaires à la piste trak */
    public void AddSilence(int trak,int duree) {
        zemidi.AddSilence(trak, duree);
    }
    
    /** Définit l' instrument de la piste trak (l' instrument doit être compris entre 1 et 128 et correspond à la base MIDI 1*/
    ...
    public void AddPerc(int percu, int duree, boolean ensemble) {                       // A FAIRE : 
        zemidi.AddPerc(percu, duree, ensemble);                                         // Trouver un meilleur moyen de gérer les percus
